    ...
    set origin_txindiv_x [expr [snap_to_grid 233 $horiz_pitch]]
    set origin_txindiv_y [expr [snap_to_grid 203 $vert_pitch]]
    ...
    set origin_minv_x [snap_to_grid 520.9 $horiz_pitch]
    set origin_minv_y [snap_to_grid 198.1 $vert_pitch] 
...
# Use automatic floorplan synthesis to pack macros (e.g., SRAMs) together
    ...
    placeInstance \
    minv_i/inv_1_fixed \
    [snap_to_grid 520.9 $horiz_pitch] \
    [snap_to_grid 198.1 $vert_pitch] \
    ...
    for {set k 0} {$k<4} {incr k} {
        placeInstance \
        itx/iPI_$k\__iPI \
            [expr $origin_txpi_x + ($pi_neighbor_spacing)*($k)]  \
            $origin_txpi_y 
    }
    #placeInstance \
    #itx/iPI_1__iPI \
    ...
    for {set k 0} {$k<4} {incr k} {
    	createPlaceBlockage -box \
        	[expr $origin_txpi_x + $pi_neighbor_spacing*($k) -  $blockage_width]  \
        	[expr $origin_txpi_y - $blockage_height]\
        	[expr $origin_txpi_x + ($pi_neighbor_spacing)*($k) + $pi_width + $blockage_width]  \
        	[expr $origin_txpi_y + $pi_height + $blockage_height]
    }
	#Memory Blockage
    createPlaceBlockage -box \
